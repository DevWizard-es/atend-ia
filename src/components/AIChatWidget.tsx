...
import { MessageCircle, X, Send, Bot, Zap, Smile, Shield } from "lucide-react";
...
export default function AIChatWidget({ businessName, agentTone = "Pro" }: AIChatWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] font-sans">
      ...
      <button
        ...
        className={cn(
          "w-16 h-16 rounded-full bg-blue-600 shadow-2xl shadow-blue-500/40 flex items-center justify-center text-white hover:scale-110 active:scale-95 transition-all duration-300 group overflow-hidden relative",
          isOpen && "rotate-90 opacity-0 scale-0 pointer-events-none"
        )}
      >
      ...
      <div
        className={cn(
          "absolute bottom-0 right-0 w-[min(400px,calc(100vw-2rem))] bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden transition-all duration-500 origin-bottom-right transform scale-0 opacity-0 pointer-events-none flex flex-col h-[500px]",
      ...
      >
        ...
        <div className="p-5 bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-between text-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-black text-sm tracking-tight leading-none mb-1">Asistente IA</div>
              <div className="flex items-center gap-1.5 opacity-80">
                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
                <span className="text-[10px] font-bold uppercase tracking-widest leading-none">En línea</span>
              </div>
            </div>
          </div>
          <button
